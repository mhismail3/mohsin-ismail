...
Claude Code is obviously an amazing piece of software. But I'd love to be able to have access to it, running on my own machine with my own filesystem, on the go. So I spent the last couple of days attempting to build a way to access Claude Code remotely from my phone. The idea: text Claude, have it work on my codebase, and see the results stream back with persistent access to my computer at home. I called it [Claude Moose](https://github.com/mhismail3/claude-moose). Here's what I learned so far.
...
I pivoted to Discord^[I asked Claude what are the best platforms for this type of messaging and evaluated a few of the options - Discord stood out] and had Opus 4.5 rewrite the implementation in Python. The same core issues followed me: misformatted terminal output, the bot failing to recognize inputs, sessions hanging unexpectedly.
